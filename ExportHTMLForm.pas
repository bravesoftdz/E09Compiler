...
begin
	...
	WriteLn(f, '</head>');
	WriteLn(f, '<body><p><table border="' + txtBorder.Text + '" cellspacing="0" cellpadding="2" bordercolor="#000000">');
	for i := 0 to grid.RowCount - 1 do begin
		...
		for j := 0 to grid.ColCount - 1 do
			if grid.Cells[j, i] = '' then
				...
				WriteLn(f, #9#9'<td bordercolor="#000000"><font face="' +  cmbFontFace.Text + '" size="' + txtFontSize.Text + '">' + grid.Cells[j, i] + '</font></td>');
		WriteLn(f, #9'</tr>');
	end;
